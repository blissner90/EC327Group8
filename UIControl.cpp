...
// script to implement the OnTrack Weekly Planner in SFML with full functionality
...
// getCurrTimeSec() uses and returns the number of seconds which have passed since EPOCH as an int for program reset purposes
int getCurrTimeSec(){

  vector<string> daysOfWeekVec{"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};  // vector of day abbreviations

  time_t epochtime = time(NULL);
    std::tm etime = *localtime(&epochtime);
    string currentDayStr = (ctime(&epochtime));
    currentDayStr = currentDayStr.substr(0,3);  // get the day abbreviation from the string representation of current time
    int currentDay;

    for (int i = 0; i < daysOfWeekVec.size(); i++){
        if (currentDayStr.compare(daysOfWeekVec.at(i)) == 0) currentDay = i;  // get current day index
      }

    // convert current day and time to time since EPOCH
...
// creation of default profile/settings page using the profile information class and a vector of vector of task classes representing
// the tasks for each day
int loadDefaultProfile(Profile& userInformation, vector<vector<Task>>& taskInformation){

  // create and format all basic shapes which appear as part of the User Interface
  ...
  // create and format all text which appears as part of the User Interface
  sf::Font font;
  ...
  while (mainWindow.isOpen()) {
    ...
    while (mainWindow.pollEvent(event)) {
      if (event.type == sf::Event::Closed){
        mainWindow.close();  // if window is closed, return -1 which represents closed window state
        return -1;
      }
    }

    mainWindow.setFramerateLimit(20);  // set frame rate
    ...
    if (backButton.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      ...
      if (event.type == sf::Event::MouseButtonPressed) return 0;  // if button clicked, go back to main default planner (return 0 code)
    }
    ...
    if (FAQ.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      ...
      if (event.type == sf::Event::MouseButtonPressed) return 1000;  // if FAQ button clicked, go to FAQ page (return 1000 code)
    }

    postItLabel.setFillColor(Color::Black);
    if (userInformation.level >= 5){  // unlocks at user level 5
      if (postItLabel.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
          userInformation.theme = 1;  // update theme in profile class
          return 3;  // if clicked, go to post it theme profile page (return code 3)
        }
      }
    }
    else {
    ...
    if (userInformation.level >= 10){  // unlocks at level 10
      if (darkLabel.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
          userInformation.theme = 2;  // update theme in profile class
          return 5;  // if clicked, go to dark theme profile page (return code 5)
        }
      }
    }
    else {
    ...
    for (int i = 0; i < 6; i++){  // individual avatar color and theme elements
      int j = i + 3;
      ...
      if ((userInformation.level < 15) && (i < 3)){  // less than level 15 case
      ...
      if (possibleColors.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) userInformation.avatarColor = i;  // update color of avatar when clicked
      }
      if (possibleShapes.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) userInformation.avatarShape = j;  // update shape of avatar when clicked
        }
      }
      else if (userInformation.level >= 15){  // unlocks at level 15
        mainWindow.draw(possibleColors);
        mainWindow.draw(possibleShapes);
...
// creation of default main planner page using the profile information class and a vector of vector of task classes representing
// the tasks for each day
int loadDefaultMainPlanner(Profile& userInformation, vector<vector<Task>>& taskInformation) {

  // create and format the basic shapes of the user interface
  RectangleShape basicOutline(sf::Vector2f(windowLength, windowWidth));
  ...
  // create and format text which appears on the day planner UI
  sf::Text digitalClock;
  sf::Font font;
  font.loadFromFile("/usr/share/fonts/truetype/ubuntu/Ubuntu-BI.ttf");
  digitalClock.setFont(font);
  ...
  while (mainWindow.isOpen()) {
    ...
    while (mainWindow.pollEvent(event)) {
      if (event.type == sf::Event::Closed){
        mainWindow.close();
        return -1; // close window if exit button is clicked (return 1 code)
      }
    }


    mainWindow.setFramerateLimit(20); // set frame rate for performance
    ...
    int dayIndex = 100;  // formating for day specific return code

    for (int i = 0; i < 7; i++){  // create buttons for each day
      tempButtonDays.setFillColor(Color::Blue);
      ...
      if (tempButtonDays.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        ...
        if (event.type == sf::Event::MouseButtonPressed) return dayIndex;  // go to specific default day planner (return code when specific day is selected)
      }
    ...
    if (basicAvatar.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        ...
        if (event.type == sf::Event::MouseButtonPressed) return 1;  // go to default profile page when avatar is clicked (return code 1)
    }
    int currentTime = getCurrTimeSec();

      for (int i = 0; i < 7; i++){  // format the weekly planner UI, task specific 
        ...
        char buffer[50];
        if (hourRemaining < 0) {  // overdue case (text becomes red)
        ...
        else if (hourRemaining <= 2){  // close to being due case (text becomes orange)
        ...
        if (taskInformation.at(i).at(j).complete) {  // if task is complete, remove time and format as green
    ...
    if (addTask.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        ...
        if (event.type == sf::Event::MouseButtonPressed) return 2000;  // if add task is clicked, go to add task page (return code 2000)
    }
...
// creation of default day planner page using the profile information class, a vector of vector of task classes representing
// the tasks for each day, and the selected day as a string
int loadDefaultDayPlanner(Profile& userInformation, vector<vector<Task>>& taskInformation, string day){
  ...
  for (int i = 0; i < daysOfWeek.size(); i++){
    if ((daysOfWeek.at(i)).compare(day) == 0) dayIndex = i; // determine the current day as an index
  }

  int lengthSpecific = taskInformation.at(dayIndex).size();

  // create and format the major shapes of the UI
  RectangleShape basicOutline(sf::Vector2f(windowLength, windowWidth));
  ...
  // create and format the text of the UI
  sf::Text digitalClock;
  sf::Font font;
  font.loadFromFile("/usr/share/fonts/truetype/ubuntu/Ubuntu-BI.ttf");
  digitalClock.setFont(font);
  ...
  while (mainWindow.isOpen()) {
    ...
    while (mainWindow.pollEvent(event)) {
      if (event.type == sf::Event::Closed){
        mainWindow.close();
        return -1;   // close window if exit button is clicked (return code -1)
      }
    }
    ...
    if (tempButtonDays.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      tempButtonDays.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return 0;  // if back button is clicke, go to default week planner (return code 0)
    }

    // draw major UI features
    mainWindow.draw(basicOutline);
    mainWindow.draw(mainBox);
    mainWindow.draw(horizontalLine);
    mainWindow.draw(tempButtonDays);
    ...
    if (basicAvatar.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        basicAvatar.setOutlineColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return 1;  // if avatar is clicked, go to default profile page (return code 1)
    }
    // draw more components
    mainWindow.draw(basicAvatar);
    mainWindow.draw(avatarExample);
    mainWindow.draw(digitalClock);
    mainWindow.draw(headerUser);
    ...
    if (addTask.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        addTask.setFillColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return 2000;  // if add task is selected, go to add task page (return code 2000)
    }
    ...
    for (int i = 0; i < taskInformation.at(dayIndex).size(); i++){
        ...
        if (taskInformation.at(dayIndex).at(i).complete) { // determine whether task has been marked as complete and indicate
          ...
          if (unmark.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
            if (event.type == sf::Event::MouseButtonPressed) {  // if unclicked, remove the points which were added when marked as complete
              ...
              taskInformation.at(dayIndex).at(i).pointsgiven = 0;  // update the task class
            }
          }
        }
        else {  // task not marked as complete case
          ...
          if (checkbox.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
            if (event.type == sf::Event::MouseButtonPressed) {  // if clicked, add points and format the profile class to indicate point update
              taskInformation.at(dayIndex).at(i).complete = !taskInformation.at(dayIndex).at(i).complete;
              if ((currDiffernceSec/3600) > 12) addedPoints = 25 + (2 * currDiffernceSec/3600);  // bonus points if more than 12 hours early
              else if (currDiffernceSec < 0) addedPoints = 0; // no points if late
              else addedPoints = 25;  // default 25 poits 
              taskInformation.at(dayIndex).at(i).pointsgiven = addedPoints;  // update task class with how many points awarded
              userInformation.updatePoints(taskInformation.at(dayIndex).at(i).pointsgiven);
            }
          }
        }

        // format day planner specific text with info from task class
        ...
        if (deleteTask.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){   // delete a task feature
            if (event.type == sf::Event::MouseButtonPressed) {  // prompt confirmation of task deletion
        ...
        if (confirmation.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
            if (event.type == sf::Event::MouseButtonPressed) {  // delete task if clicked again
...
// creation of postit theme profile/settings page, ssame structure as loadDefaultProfile(), so please look at those comments to
...
int loadPostItProfile(Profile& userInformation, vector<vector<Task>>& taskInformation){
  ...
  while (mainWindow.isOpen()) {
    ...
    if (backButton.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      backButton.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return 2;  // unique post it main planner return code
    }
...
// creation of postit theme main planner page, same structure as loadDefaultMainPlanner(), so please look at those comments to
// understand functionality
int loadPostItMainPlanner(Profile& userInformation, vector<vector<Task>>& taskInformation) {

  // vectors for formatting the colors of the postit theme
  ...
  while (mainWindow.isOpen()) {
    ...
    std::time_t stdTime = std::time(NULL);
    std::tm stdNow = *std::localtime(&stdTime);
    ...
    if (basicAvatar.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        basicAvatar.setOutlineColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return 3;  // unique return code for post-it profile page
    }

    int dayIndex = 200;  // day index in 200s for posit-it theme
    ...
    for (int i = 0; i < 4; i++){
      ...
      if (tempButtonDays.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        tempButtonDays.setFillColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return dayIndex;  // unique return code for post it day planner page
        }
      mainWindow.draw(tempButtonDays);
      dayIndex += 1;
...
// creation of postit theme day planner page, same structure as loadDefaultDay Planner(), so please look at those comments to
// understand functionality
int loadPostItDayPlanner(Profile& userInformation, vector<vector<Task>>& taskInformation, string day){
  ...
  while (mainWindow.isOpen()) {
    ...
    if (tempButtonDays.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      tempButtonDays.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return 2;  // unique return code for post it main planner page
    }
...
// creation of dark theme profile/settings page, ssame structure as loadDefaultProfile(), so please look at those comments to
// understand functionality
int loadDarkProfile(Profile& userInformation, vector<vector<Task>>& taskInformation){
  ...
  while (mainWindow.isOpen()) {
    ...
    if (backButton.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      backButton.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return 4;  // unique dark theme main page return code
    }
    ...
    if (userInformation.level >= 5){
      if (postItLabel.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        ...
        if (event.type == sf::Event::MouseButtonPressed) {
          ...
          return 3;  // unique post it theme profile return code
        }
      }
    }
    else {
    ...
    if (defaultLabel.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      ...
      if (event.type == sf::Event::MouseButtonPressed) {
        userInformation.theme = 0;
        return 1;  // unique default theme profile return code
      }
    }
...
// creation of dark theme main planner, same structure as loadDefaultMainPlanner(), so please look at those comments to
// understand functionality
int loadDarkMainPlanner(Profile& userInformation, vector<vector<Task>>& taskInformation) {
  ...
  while (mainWindow.isOpen()) {
    ...
    int dayIndex = 300;  // day index in 300s for dark theme
    ...
    for (int i = 0; i < 7; i++){
      ...
      if (tempButtonDays.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        tempButtonDays.setFillColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return dayIndex;  // return unique dark theme day planner return code
      }
      mainWindow.draw(verticalLine);
      mainWindow.draw(tempButtonDays);
      dayIndex += 1;
    ...
    if (basicAvatar.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        basicAvatar.setOutlineColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return 5;  // return unique dark theme profile page return code
    }
...
// creation of dark theme day planner page, ssame structure as loadDefaultDayPlanner(), so please look at those comments to
// understand functionality
int loadDarkDayPlanner(Profile& userInformation, vector<vector<Task>>& taskInformation, string day){
  ...
  while (mainWindow.isOpen()) {
    ...
    if (tempButtonDays.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      tempButtonDays.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return 4;  // unique dark theme weekly planner return code
    }
    mainWindow.draw(basicOutline);
    mainWindow.draw(mainBox);
    mainWindow.draw(horizontalLine);
    ...
    if (basicAvatar.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        basicAvatar.setOutlineColor(Color::Green);
        if (event.type == sf::Event::MouseButtonPressed) return 5;  // unique dark theme profile page return code
    }
    mainWindow.draw(basicAvatar);
    mainWindow.draw(avatarExample);
    mainWindow.draw(digitalClock);
...
// creation of the AddTask page of the UI using the profle and task classes
int loadAddTask(Profile& userInformation, vector<vector<Task>>& allTaskData) {

  // format shapes which appear on UI
  CircleShape backButton(20, 3);
  backButton.setPosition(50, 60);
  backButton.rotate(270);

  // creation and formating of text which appears on UI
  sf::Text morePrompt;
  sf::Font font;
  font.loadFromFile("/usr/share/fonts/truetype/ubuntu/Ubuntu-BI.ttf");
  ...
  // vectors to store the user's choice of due time
  ...
  // creation and formating of textboxes
  ...
  while (mainWindow.isOpen()) {
    ...
    if (backButton.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      backButton.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return (userInformation.theme * 2);  // go back to main planner page if back button is selected
    }
    ...
    for (int i = 0; i < 7; i++){
      ...
      if (day.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) selectedDay = i;  // store the selected day
      }
      if (selectedDay == i){
    ...
    for (int i = 0; i < 12; i++){
      ...
      if (listTime.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) selectedHour = i;  // store the selected hour
      }
      if (selectedHour == i){
    ...
    for (int i = 0; i < 2; i++){
      ...
      if (ampm.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) selectedAPM = i;  // store if AM/PM
      }
      if (selectedAPM == i){
    ...
    if ((taskNameTB.getText().compare("") == 0) || (taskDescTB.getText().compare("") == 0) || (selectedDay == -1) || (selectedHour == -1) || (selectedAPM == -1)){
        ...
        isCompleted.setString("ALL FIELDS REQUIRED BEFORE SUBMITTING");  // report error to user
      ...
      if (currentTime > taskDue){  // check if user is trying to set a deadline in the past, report error to user
        isCompleted.setPosition(900, 840);
        isCompleted.setString("ATTEMPTING TO SET DUE DATE IN THE PAST, PLEASE CHECK INPUTS");
        isCompleted.setCharacterSize(20);
        isCompleted.setFillColor(Color::Red);
        }
      else if (allTaskData.at(selectedDay).size() == 4){  // check if max tasks per day (4) has been reached, report error to user
        ...
        if (taskDescTB.getText().compare("UUDDLRLRBA") == 0) isCompleted.setString("PROCEED WITH CAUTION:");  // god mode EASTER EGG checker
        ...
        if (isCompleted.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
          ...
          if (event.type == sf::Event::MouseButtonPressed){
            if (taskDescTB.getText().compare("UUDDLRLRBA") == 0) return -2;  // if GOD MODE, return unique code
            Task createdTask = Task(taskNameTB.getText(), taskDescTB.getText(), selectedDay, selectedHour, selectedAPM);  // otherwise create task class with inputted data
            allTaskData.at(selectedDay).push_back(createdTask); // push task onto the vector, sorting in by due time
            ...
            return userInformation.theme * 2;  // return to current theme's main planner page
          }
        }
        }
      }
...
// creation of the first time user screen for profile information collection
int loadFirstTimeUser() {

  // create and format basic shapes of UI
  ...
  // create and format textboxes of the UI
  ...
  while (mainWindow.isOpen()) {
    ...
    while (mainWindow.pollEvent(event)) {
      if (event.type == sf::Event::Closed){
        mainWindow.close();
        return -1; // close window and return close window code
      }
      if (event.type == sf::Event::TextEntered){
    ...
    if (selectedColor != -1) { // draw outline if color has been selected
    ...
    if (selectedShape != -1){  // draw outline if shape has been selected
    ...
    for (int i = 0; i < 3; i++){  // format and draw possible shapes and colors
      int j = i + 3;
      ...
      if (possibleColors.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) selectedColor = i;  // update selected color index
      }
      if (possibleShapes.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
        if (event.type == sf::Event::MouseButtonPressed) selectedShape = j;  // update selected shape index
      }
    }
    // check to make sure all required fields are inputted
    if ((selectedShape == -1) || (selectedColor == -1) || (firstNameTB.getText().compare("") == 0) || (lastNameTB.getText().compare("") == 0)){
        isCompleted.setString("ALL FIELDS REQUIRED BEFORE SUBMITTING");  // prompt user with error message
        isCompleted.setFillColor(Color::Red);
        isCompleted.setCharacterSize(20);
      }
    else{ // all field inputted, accept inputs
        isCompleted.setString("CONTINUE:");
        isCompleted.setFillColor(Color::White);
        isCompleted.setCharacterSize(40);
        if (isCompleted.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){ // if continue button is selected
          isCompleted.setFillColor(Color::Green);
          if (event.type == sf::Event::MouseButtonPressed){
...
// creation of the FAQ page of the UI
int loadFAQ(Profile& userInformation){
  ...
  while (mainWindow.isOpen()){
    ...
    while (mainWindow.pollEvent(event)) {
      if (event.type == sf::Event::Closed){
        mainWindow.close();
        return -1;  // return code for closed window
      }
    }
    ...
    if (backButton.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){ 
      backButton.setFillColor(Color::Green);
      if (event.type == sf::Event::MouseButtonPressed) return (userInformation.theme * 2 + 1);  // if back button selected, return to current theme's profile page
    }

    // draw UI components to window
...
// creation of GOD MODE Easter Egg profile page
void loadGodMode() {

  // create and format basic UI shapes
  RectangleShape basicOutline(sf::Vector2f(windowLength, windowWidth));
  ...
  // load in and format the photo for GOD MODE
  ...
  while (mainWindow.isOpen()) {
    ...
    while (mainWindow.pollEvent(event)) {
      if (event.type == sf::Event::Closed){
        mainWindow.close();
        return;  // close window when clicked
      }
    }
    ...
    // change buttons to green when region is hovered over
    backButton.setFillColor(Color::Black);
    if (backButton.getGlobalBounds().contains(mainWindow.mapPixelToCoords(sf::Mouse::getPosition(mainWindow)))){
      backButton.setFillColor(Color::Green);
    }
    ...
    // create and format the possible shapes and colors
    for (int i = 0; i < 6; i++){
      int j = i + 3;
      CircleShape possibleShapes(30, j);
      possibleShapes.setPosition(505 + i*100, 810);
    ...
    // on click, add location of click to a vector of locations
    ...
    for (int i = 0; i < savedSpritePositions.size(); i++){  // draw another Prof. Carruthers in the location of the click
...
// initialStartUp() handles all of the switching between the different screens of the UI and runs infinitely until window is closed
int initialStartUp() {
  ...
  string dataLine;
  vector<string> daysOfWeek{"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
  // vector of functions utilized in screen changes
  ...
  if (ifile.peek() == std::ifstream::traits_type::eof()) isNewUser = true;  // if saved data file is empty, it is a first time user

  int closedFirstTime = 0;

  if (isNewUser) closedFirstTime = loadFirstTimeUser();  // run first time user if the user is a first time user

  if (closedFirstTime == -1) return 0; // check if user simply closed the profile creation screen


  std::ifstream dataFile("savedData.txt");  // re-read the savedData (will now never be empty)
    while (std::getline(dataFile, dataLine)) {  // loop through each line
      int lengthStr = dataLine.length() + 1;
      char lineAsCharArr[lengthStr];
      strcpy(lineAsCharArr, dataLine.c_str());

      if (dataLine.substr(0,4).compare("Prof") == 0){ // profile class line, extract profile data
      ...
      if (dataLine.substr(0,4).compare("Task") == 0) {  // task class line, extract task data
        char *token = strtok(lineAsCharArr, "\t");
        ...
        Task nTask = Task(taskLine.at(1), taskLine.at(2), stoi(taskLine.at(3)), stoi(taskLine.at(4)), stoi(taskLine.at(5)), bool(stoi(taskLine.at(6))), stoi(taskLine.at(7))); // create task class from task data
        allTaskData.at(stoi(taskLine.at(3))).push_back(nTask);  // push onto vector of vector of tasks
      }
    }

  std::replace(profileLine.at(1).begin(), profileLine.at(1).end(), '.', ' ');
  Profile allUserInformation = Profile(profileLine.at(1), stoi(profileLine.at(2)), stoi(profileLine.at(3)), stoi(profileLine.at(4)), stoi(profileLine.at(5)), stoi(profileLine.at(6)), stoi(profileLine.at(7)));  // create profile class from profile data
  
  int nextState = vectorOfFunc.at(allUserInformation.theme * 2)(allUserInformation, allTaskData);  // loads the last saved theme's default main planner
  while (nextState >= 0){  // loops until exit or god mode
    if (nextState < 10) nextState = vectorOfFunc.at(nextState)(allUserInformation, allTaskData);  // uses unique return codes to dictate which screen to next load
  ...
  // upon GodMode or exit, write all profile and task data to an output file for future openings
  std::ofstream ofile;
  ...
  ofile << to_string(epochtime) <<"\t" << currentDayStr << "\n";  // add the time and day of last opening, utilized to check when to reset the data each week

  if (nextState == -2){ // load GodMode if needed
    loadGodMode();
  }
